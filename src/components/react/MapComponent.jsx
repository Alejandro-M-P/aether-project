...
// Eliminamos importaciones que no uses si es necesario, pero mantenemos las tuyas
...
export const MapComponent = ({ messages = [], openProfile }) => {
	...
	// --- NUEVA FUNCIÓN: ZOOM INTELIGENTE ---
	// Esta función centraliza la lógica de movimiento para no repetirla
	const handleSmartZoom = (lat, lng) => {
		if (!globeEl.current) return;

		const currentPov = globeEl.current.pointOfView();

		// Umbral de "Lejos". Si estás más alto que 0.5, se considera lejos.
		const ALTITUDE_THRESHOLD = 0.5;
		// Altura objetivo al acercarse (la que usabas antes, 0.2 o 0.25)
		const TARGET_CLOSE_ALTITUDE = 0.25;

		// Si estamos altos (> 0.5), bajamos a 0.25.
		// Si ya estamos bajos (<= 0.5), mantenemos la altura actual.
		const newAltitude =
			currentPov.altitude > ALTITUDE_THRESHOLD
				? TARGET_CLOSE_ALTITUDE
				: currentPov.altitude;
	...
	return (
		<div className="relative w-full h-full bg-[#0a0a0a] cursor-move select-none">
			<Globe
				...
				// --- CLIC EN PUNTO (3D) USANDO ZOOM INTELIGENTE ---
				onPointClick={(d) => {
				...
				htmlElement={(d) => {
					if (!markersRef.current[d.id]) {
						...
						// --- CLIC EN ICONO HTML USANDO ZOOM INTELIGENTE ---
